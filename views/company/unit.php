                            ...
                            <table id="example2" class="table table-bordered table-hover dataTable " role="grid" aria-describedby="example2_info">
                                ...
                                <tbody>
                                    ...
                                    <tr role="row" class="odd">
                                      <td><?= $k+1;?></td>
                                      <td><?= $v->unit_name;?></td>
                                      <td><?= $v->unit_status == 1 ? "正常" : "停用";?></td>
                                      <td>
                                          <a href="<?= Yii::$app->urlManager->createUrl(['company/unit_detail','id'=>$v->unit_id])?>"> 详情 </a> 
                                          &nbsp;|&nbsp;
                                           <a class="btn-del" href="javascript:;" data-message="确认要操作【<?= $v->unit_name?>】吗？" data-link="<?= Yii::$app->urlManager->createUrl(['company/unit_del','id'=>$v->unit_id])?>"><?= $v->unit_status == 1 ? "停用" : "启用"?></a>
                                      </td>
                                    </tr>
                                    <?php endforeach;?>
                                </tbody>
            ...
            <form role="form" method="post" action="<?= Yii::$app->urlManager->createUrl('company/unit_handle')?>">
              <div class="box-body">
                <div class="form-group">
                  <label >单位名称</label>
                  <input type="text" class="form-control" name="unit_name" placeholder="单位名称">
                </div>
              </div>
              <!-- /.box-body -->
